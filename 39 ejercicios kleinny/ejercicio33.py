...
def lenUltimaPalabra_kmdt(cadena):
   longitud_kmdt=len(cadena)
   if longitud_kmdt==0:
       return 0
   cantidad_kmdt=0
   for i in range(longitud_kmdt):
       if cadena[i]!=' ':
           cantidad_kmdt+=1
       else:
           if cadena[i]==' ' and i<(longitud_kmdt-1) and cadena[i+1]!=' ':
               cantidad_kmdt=0
...
def primerosTresDigitos_kmdt(numero):
   while numero >= 1000:
     numero = numero // 10
   return numero

def obtenerIdentificador_kmdt(nombre, dni):
   nombre=nombre.strip()
   id_kmdt=nombre[:nombre.find(" ")]
   id_kmdt=id_kmdt+str(lenUltimaPalabra_kmdt(nombre))
   id_kmdt=id_kmdt+str(primerosTresDigitos_kmdt(dni))
   return id_kmdt

#programa principal
nombre_kmdt=input("Nombre del socio: ")
while nombre_kmdt!="":
   dni=int(input("DNI del socio: "))
   while(DNIvalido(dni)):
      print("Número inválido.")
      dni=int(input("DNI del socio: "))
   print(obtenerIdentificador_kmdt(nombre_kmdt,dni))
   nombre_kmdt=input("Nombre del socio: ")