...
corrimiento_kmdt=int(input("Corrimiento: "))
for i in range(5):
    mensaje_kmdt=input("Mensaje a encriptar: ")
    encriptado_kmdt=""
    for caracter_kmdt in mensaje_kmdt:
        if caracter_kmdt.lower() in alfabeto_kmdt:
            indice_kmdt=alfabeto_kmdt.find(caracter_kmdt.lower())
            indice_kmdt=(indice_kmdt+corrimiento_kmdt)%27
            encriptado_kmdt+=alfabeto_kmdt[indice_kmdt]
        else:
            encriptado_kmdt+=caracter_kmdt
        print("Mensaje encriptado: ", encriptado_kmdt)